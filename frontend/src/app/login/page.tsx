...
export default function LoginPage() {
  ...
  // Estado de erros (controla mensagens por campo)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        // Cria objeto de erros de campo
        const fieldErrors: { [key: string]: string } = {};

        // Agora é `issues` (não mais `errors`)
        ...
        // Erro genérico (falha de credenciais, servidor, etc)
  ...
  // Preenche rapidamente o form para testes
  const handleQuickLogin = (type: 'admin' | 'user') => {
    if (type === 'admin') {
      setFormData({ email: 'admin@docgestor.com', password: 'admin123' });
    } else {
      setFormData({ email: 'user@docgestor.com', password: 'user123' });
    }
  };
